...
tags: [h2o, data-science, machine-learning, R]
---

# 1. Introduction
In this post we discuss the H2O machine learning platform. We discuss what this means, and how to get started with it, using R.

# 2. What is H2O?

__The definition found on H2O's Github page is a lot to take in,__ especially if you're just starting out with H2O (like I am!): ["H2O is an in-memory platform for distributed, scalable machine learning. H2O uses familiar interfaces like R, Python, Scala, Java, JSON and the Flow notebook/web interface, and works seamlessly with big data technologies like Hadoop and Spark."](https://github.com/h2oai/h2o-3)

Let's break down this statement piece by piece.

## 2.1 H2O is an In-Memory Platform

They start off by telling us that H2O is an "in-memory platform". __Saying that it's in-memory means that the data being used is loaded into main memory (RAM).__ Reading from main memory, (also known as primary memory) is typically much faster than secondary memory (such as a hard drive). 

They also call H2O a "platform." __A platform, in this context, is software which can be used to build something - in this case, machine learning models.__ 
...
__H2O can run on clusters.__ Although I haven't actually tried this (yet), I've read that H2O works on clusters such as [Hadoop](https://hadoop.apache.org/). It's something that I still need to experiment with, and when I do, I'll post a link here. So, for now, I'm just going to interpret the phrase "distributed, scalable machine learning" as saying that we can run the H2O platform on a cluster.

One thing to note, is that H2O uses what's known as a Distributed Key Value (DKV). You can read more about it [here](https://www.h2o.ai/blog/h2o-architecture/), but essentially what this means, is that __any object you create in H2O can be distributed amongst several nodes in the cluster.__ 

Of course, if you aren't using a cluster - and are just using a laptop, you need need to have enough memory to hold your objects locally. Whether you are using a cluster, or running locally, once you've imported your data into the H2O platform, __you get back a hexadecimal key which you can then use to access your data.__

So, __Distributed Key Value__: It's distributed because your object can be spread amongst several nodes in your cluster. And the key-value part means that you get back a key into a hashmap containing your object.
